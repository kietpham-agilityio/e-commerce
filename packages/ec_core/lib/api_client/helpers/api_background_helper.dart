...
class ApiBackgroundHelper {
  ...
  /// Background API call with circuit breaker pattern
  static Future<T> callApiWithCircuitBreaker<T>({
    required Future<T> Function() apiCall,
    String? errorContext,
    Duration timeout = const Duration(seconds: 30),
    ...
    return _circuitBreaker.execute<T>(
      () => callApiRunBackground<T>(
        apiCall: apiCall,
        errorContext: errorContext,
        timeout: timeout,
      ),
  ...
  static final _CircuitBreaker _circuitBreaker = _CircuitBreaker();
}

/// Circuit breaker implementation for background API calls
